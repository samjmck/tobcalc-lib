...
function parseArgs() {
  ...
  flags.lang = flags.lang.map((l) => l.toUpperCase()).filter((l) =>
...
async function fetchAssets() {
  ...
  for (const [lang, [resUrl, helpUrl]] of Object.entries(tobUrls)) {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to load TOB ${lang} pdf, check ${helpUrl} to see whether the URL changed.`,
      );
    }
    Deno.writeFile(
      `${assetsPath}TD-OB1-${lang}.pdf`,
  ...
  response = await fetch(
    "https://github.com/tecnickcom/tc-font-pdfa/raw/main/pfb/PDFAHelvetica.pfb",
  );
  if (!response.ok) {
    throw new Error(
      "Failed to load Helvetica font, check https://github.com/tecnickcom/tc-font-pdfa to see whether the URL changed.",
    );
  }
  Deno.writeFile(
    `${assetsPath}PDFAHelvetica.pfb`,
    new Uint8Array(await response.arrayBuffer()),
  );
  ...
  if (!response.ok) {
    throw new Error(
      "Failed to load sRGB color profile, check https://www.color.org/srgbprofiles.xalter to see whether the URL changed.",
    );
  }
  Deno.writeFile(
    `${assetsPath}sRGB2014.icc`,
    new Uint8Array(await response.arrayBuffer()),
  );
}
...
async function logAnnotations(
  fileName,
  subtypeFilter: Array<PDFName> | null = null,
) {
  ...
  for (const [p, pdfPage] of pdfDoc.getPages().entries()) {
    ...
    for (let a = 0; a < annots?.size(); a++) {
      ...
      if (
        !subtypeFilter ||
        subtypeFilter.some((st) =>
          st.toString() == dict.get(PDFName.of("Subtype")).toString()
        )
      ) {
        ...
        if (dict.has(PDFName.of("AP"))) {
          ...
          const apStream = pdfDoc.context.lookup(
            apStreamRef,
            PDFStream,
          ) as PDFRawStream;
...
function addStrikethroughAnnotation(
  pdfPage,
  x,
  y,
  length,
...
async function emptyForm(pdfDoc, lang) {
  ...
  for (const [p, pdfPage] of pdfDoc.getPages().entries()) {
    ...
    const texts = blocks.map((block) =>
      block.match(textPattern)?.at(-1).slice(1, -4) ?? ""
    ); // And extract the last text shown in each block
...
async function createForm(pdfDoc, lang) {
  ...
  for (const [p, pdfPage] of pdfDoc.getPages().entries()) {
    ...
    mods?.strikethroughs?.forEach((props, name) => {
      const ref = addStrikethroughAnnotation(
        pdfPage,
        props.x,
        props.y,
        props.length,
