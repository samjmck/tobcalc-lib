// Cannot use import from "pdf-lib" due to the error "The requested module 'pdf-lib' does not provide an export named 'TextAlignment'"
import { rgb, TextAlignment } from "https://cdn.skypack.dev/pdf-lib@1.17.1?dts";
...
function xChar(char) {
  ...
  return (char.charCodeAt(0) + offset).toString(16).toUpperCase().padStart(
    4,
    "0",
  );
}
...
function moveText(
  char,
  offset,
  charSub: string | undefined = undefined,
): CharSubs {
  return new Map([
...
function moveTextAround(
  ...
  return new Map([
    ...
    [
      char,
      `><${xSPACE}>${spaceOffset + offsetBefore}<${
        charSub ?? char
      }>${offsetAfter}<`,
    ],
  ]);
...
function capitalize(...words: string[]): CharSubs {
  ...
  words.forEach((word) =>
    word.toLowerCase().split("").forEach((c) => chars.add(c))
  );
  ...
  "DE": {
    0: { // First page
      ...
      replaceText: new Map([
        ...
        [
          17,
          new Map([ // Capitalize "Börsengeschäfte"
      ...
      strikethroughs: new Map([
        ...identificationStrikethroughs(
          305,
          610, // (X, Y) position of the lowest identification strikethrough line (inst)
  ...
  "EN": {
    0: { // First page
      ...
      strikethroughs: new Map([
        ...identificationStrikethroughs(
          307,
          679, // (X, Y) position of the lowest identification strikethrough line (inst)
