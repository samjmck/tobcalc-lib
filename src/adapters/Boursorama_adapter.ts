...
export const BoursoramaAdapter: BrokerAdapter = async (data) => {
  ...
  if (dateColumnIndex === -1) {
    throw new InformativeError(
      "boursorama_adapter.date_column_index",
      columnNames,
    );
  }
  if (isinColumnIndex === -1) {
    throw new InformativeError(
      "boursorama_adapter.isin_column_index",
      columnNames,
    );
  }
  if (valueColumnIndex === -1) {
    throw new InformativeError(
      "boursorama_adapter.value_column_index",
      columnNames,
    );
  }
  ...
  for (const rowString of rows.slice(1)) {
    ...
    if (row[isinColumnIndex] === undefined) {
      throw new InformativeError("boursorama_adapter.isin_undefined", {
        row,
        columnNames,
      });
    }
    if (row[valueColumnIndex] === undefined) {
      throw new InformativeError("boursorama_adapter.value_undefined", {
        row,
        columnNames,
      });
    }
