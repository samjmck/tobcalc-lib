...
export const Trading212Adapter: BrokerAdapter = async (data) => {
  ...
  const currencyCodeColumnIndex = columnNames.indexOf(
  ...
  if (actionColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.action_column_index",
      columnNames,
    );
  }
  if (dateColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.date_column_index",
      columnNames,
    );
  }
  if (isinColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.isin_column_index",
      columnNames,
    );
  }
  if (currencyCodeColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.currency_code_column_index",
      columnNames,
    );
  }
  if (numberOfSharesColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.number_of_shares_column_index",
      columnNames,
    );
  }
  if (pricePerShareColumnIndex === -1) {
    throw new InformativeError(
      "trading212_adapter.prices_per_share_column_index",
      columnNames,
    );
  }
  ...
  for (const rowString of rows.slice(1)) {
    ...
    if (row[numberOfSharesColumnIndex] === undefined) {
      throw new InformativeError(
        "trading212_adapter.number_of_shares_undefined",
        { row, columnNames },
      );
    }
    if (row[pricePerShareColumnIndex] === undefined) {
      throw new InformativeError(
        "trading212_adapter.price_per_share_undefined",
        { row, columnNames },
      );
    }
