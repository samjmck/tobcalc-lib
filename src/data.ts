...
export async function getExchangeRatesMap(
  currencyPeriods: { start: Date; end: Date; currencyCode: CurrencyCode }[],
): Promise<ExchangeRatesMap> {
  const promises: Promise<void>[] = [];
  for (const { start, end, currencyCode } of currencyPeriods) {
    promises.push(
      getCurrencyExchangeRatesMap(start, end, currencyCode).then(
        (currencyMap) => {
...
export async function getCurrencyExchangeRatesMap(
  start: Date,
  end: Date,
  ...
  const response = await fetch(
    `${ECB_URL_START}/service/data/EXR/D.${requestCurrencyCode}.EUR.SP00.A?${urlParamsString}`,
  );
  if (response.status !== 200) {
    throw new Error(
      `response from ECB RESTful API returned status code ${response.status}`,
    );
  }
  ...
  if (!foundTimePeriods) {
    throw new Error(
      `could not find time periods for start date ${startPeriod}, end date ${endPeriod} and currencyCodes ${currencyCode}`,
    );
  }
  ...
  for (let i = 0; i < timePeriods.length; i++) {
    ...
    let exchangeRate = <number> json.dataSets[0]
      .observations[`0:0:0:0:0:${i}`][0];
  ...
  for (
    ...
    if (!currencyMap.has(date)) {
      ...
      if (!found) {
        console.warn(
          `Cannot go back more than 5 days for date ${date} and currency code ${currencyCode}`,
        );
      }
    }
  }
...
export async function getDefaultSecuritiesMap(
  isins: string[],
  ...
  for (const isin of isins) {
    if (!securitiesMap.has(isin)) {
      promises.push(
        getSecurity(isin).then((security) => {
...
export async function getSecurity(isin: string): Promise<Security> {
  const response = await fetch(
    `${YAHOO_FINANCE_QUERY1_URL_START}/v1/finance/search?q=${isin}&quotesCount=1&newsCount=0`,
  );
  ...
  const name = json.quotes[0].longname !== undefined
    ? json.quotes[0].longname
    : json.quotes[0].shortname;
  ...
  switch (quoteType) {
    ...
    case "ETF":
      const response = await fetch(
        `${JUSTETF_URL_START}/uk/etf-profile.html?isin=${isin}`,
        {
          "method": "GET",
        },
      );
      const html = await response.text();
      const accumulating = /<td class="val" data-testid="tl_etf-basics_value_distribution-policy">Accumulating<\/td>/g.test(
        html,
      );
      return {
    ...
    default:
      throw new InformativeError("security.fetch.unknown_quote_type", {
        isin,
        json,
      });
  }
