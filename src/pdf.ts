...
function removeAnnotation(page: PDFPage, annotRef: PDFRef) {
  ...
  const apRef = (annot.get(PDFName.of("AP")) as PDFDict).get(
    PDFName.of("N"),
  ) as PDFRef;
...
async function getEmbeddedFont(doc: PDFDocument, fontRef: PDFRef) {
  ...
  const fontCID = doc.context.lookup(
    (fontDict.get(PDFName.of("DescendantFonts")) as PDFArray).get(0) as PDFRef,
    PDFDict,
  );
  const fontDescr = doc.context.lookup(
    fontCID.get(PDFName.of("FontDescriptor")) as PDFRef,
    PDFDict,
  );
  const fontFileStream = doc.context.lookup(
    fontDescr.get(PDFName.of("FontFile2")) as PDFRef,
  ) as PDFRawStream;
...
export async function fillPdf(
  ...
  const font = await getEmbeddedFont(
    document,
    tobRefs.get(PDFName.of("Font")) as PDFRef,
  );
  ...
  const nationalRegistrationNumberField = form.getTextField(
  ...
  const tableATax012QuantityField = form.getTextField(
    "table_a_tax_012_quantity",
  );
  const tableATax012TaxBaseField = form.getTextField(
    "table_a_tax_012_tax_base",
  );
  const tableATax012TaxValueField = form.getTextField(
    "table_a_tax_012_tax_value",
  );

  const tableATax035QuantityField = form.getTextField(
    "table_a_tax_035_quantity",
  );
  const tableATax035TaxBaseField = form.getTextField(
    "table_a_tax_035_tax_base",
  );
  const tableATax035TaxValueField = form.getTextField(
    "table_a_tax_035_tax_value",
  );

  const tableATax132QuantityField = form.getTextField(
    "table_a_tax_132_quantity",
  );
  const tableATax132TaxBaseField = form.getTextField(
    "table_a_tax_132_tax_base",
  );
  const tableATax132TaxValueField = form.getTextField(
    "table_a_tax_132_tax_value",
  );
  ...
  setFieldText(
    startMonthField,
    `${params.start.getMonth() + 1}`.padStart(2, "0"),
    font,
  );
  setFieldText(startYearField, `${params.start.getFullYear()}`, font);
  setFieldText(
    endMonthField,
    `${params.end.getMonth() + 1}`.padStart(2, "0"),
    font,
  );
  setFieldText(endYearField, `${params.end.getFullYear()}`, font);

  setFieldText(
    nationalRegistrationNumberField,
    params.nationalRegistrationNumber,
    font,
  );
  ...
  setFieldText(
    tableATax012QuantityField,
    `${params.tableATax012Quantity}`,
    font,
  );
  setFieldText(
    tableATax012TaxBaseField,
    formatMoney(params.tableATax012TaxBase, moneyCur, moneySep),
    font,
  );
  setFieldText(
    tableATax012TaxValueField,
    formatMoney(params.tableATax012TaxValue, moneyCur, moneySep),
    font,
  );

  setFieldText(
    tableATax035QuantityField,
    `${params.tableATax035Quantity}`,
    font,
  );
  setFieldText(
    tableATax035TaxBaseField,
    formatMoney(params.tableATax035TaxBase, moneyCur, moneySep),
    font,
  );
  setFieldText(
    tableATax035TaxValueField,
    formatMoney(params.tableATax035TaxValue, moneyCur, moneySep),
    font,
  );

  setFieldText(
    tableATax132QuantityField,
    `${params.tableATax132Quantity}`,
    font,
  );
  setFieldText(
    tableATax132TaxBaseField,
    formatMoney(params.tableATax132TaxBase, moneyCur, moneySep),
    font,
  );
  setFieldText(
    tableATax132TaxValueField,
    formatMoney(params.tableATax132TaxValue, moneyCur, moneySep),
    font,
  );

  setFieldText(
    tableATotalTaxValue,
    formatMoney(params.tableATotalTaxValue, moneyCur, moneySep),
    font,
  );
  setFieldText(
    totalTaxValue,
    formatMoney(params.totalTaxValue, moneyCur, moneySep),
    font,
  );
  ...
  const strikethroughRef = (tobRefs.get(PDFName.of("Strikethrough")) as PDFDict)
    .get(PDFName.of(identification)) as PDFRef;
