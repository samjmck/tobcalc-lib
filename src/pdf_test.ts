...
Deno.test({
  ...
  fn: async () => {
    const pdfFile = await Deno.readFile(
      "pdfs/build/TD-OB1-NL-empty-fillable-pdfa.pdf",
    );
    ...
    assertEquals(
      form.getTextField("national_registration_number").getText(),
      params.nationalRegistrationNumber,
    );
    assertEquals(form.getTextField("full_name").getText(), params.fullName);

    assertEquals(
      form.getTextField("address_line_1").getText(),
      params.addressLine1,
    );
    assertEquals(
      form.getTextField("address_line_2").getText(),
      params.addressLine2,
    );
    assertEquals(
      form.getTextField("address_line_3").getText(),
      params.addressLine3,
    );

    assertEquals(
      form.getTextField("table_a_tax_012_quantity").getText(),
      `${params.tableATax012Quantity}`,
    );
    assertEquals(
      form.getTextField("table_a_tax_012_tax_base").getText(),
      "€ 100,00",
    );
    assertEquals(
      form.getTextField("table_a_tax_012_tax_value").getText(),
      "€ 1,00",
    );

    assertEquals(
      form.getTextField("table_a_tax_035_quantity").getText(),
      `${params.tableATax035Quantity}`,
    );
    assertEquals(
      form.getTextField("table_a_tax_035_tax_base").getText(),
      "€ 200,00",
    );
    assertEquals(
      form.getTextField("table_a_tax_035_tax_value").getText(),
      "€ 2,00",
    );

    assertEquals(
      form.getTextField("table_a_tax_132_quantity").getText(),
      `${params.tableATax132Quantity}`,
    );
    assertEquals(
      form.getTextField("table_a_tax_132_tax_base").getText(),
      "€ 300,00",
    );
    assertEquals(
      form.getTextField("table_a_tax_132_tax_value").getText(),
      "€ 3,00",
    );

    assertEquals(
      form.getTextField("table_a_total_tax_value").getText(),
      "€ 5,00",
    );
    ...
    assertEquals(
      form.getTextField("signer").getText(),
      `${params.signatureName}  , ${params.signatureCapacity}`,
    );
